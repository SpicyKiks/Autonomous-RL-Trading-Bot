...
def _iter_root_files(root: Path) -> Iterable[CollectedFile]:
    ...
    if not _has_excluded_dir(self_rel) and not _is_excluded_file(self_rel, self_path):
...
def main() -> int:
    ...
    required_prefixes = [f"{d}/" for d in INCLUDE_DIRS]
    missing = [p for p in required_prefixes if not any(fi.rel_path.as_posix().startswith(p) for fi in files)]
    if missing:
        print("[make_code_zip] ERROR: Missing required folders in zip:")
        for m in missing:
            print(f"  - {m}")
        return 3
